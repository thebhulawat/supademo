...
function isInteractable(element) {
  ...
  // New: Check for custom elements that might be interactive
  if (tagName.includes('-')) {
    return true;
  }

  // New: Check for elements with click handlers
  if (element.onclick || element.addEventListener) {
    return true;
  }

  // New: Check for elements that look like buttons
  const computedStyle = window.getComputedStyle(element);
  if (
  ...
  // New: Check for elements with specific roles
  const role = element.getAttribute('role');
  if (
    role &&
    ['button', 'link', 'menuitem', 'option', 'tab'].includes(role.toLowerCase())
  ) {
    return true;
  }

  // New: Check parent elements for interactivity
...
function drawBoundingBoxes(elements) {
  console.log('drawBoundingBoxes called with', elements.length, 'elements');

  // Create a container for the bounding boxes if it doesn't exist
  let container = document.getElementById('boundingBoxContainer');
  if (!container) {
  ...
  elements.forEach((element, index) => {
    if (element.rect) {
      ...
      box.style.left = `${element.rect.left}px`;
      box.style.top = `${element.rect.top}px`;
      box.style.width = `${element.rect.width}px`;
      box.style.height = `${element.rect.height}px`;

      // Add a label with the element's ID
      ...
      console.warn('Element', index, 'has no rect property');
    }
  });

  console.log('Drew', container.childElementCount, 'bounding boxes');
}
